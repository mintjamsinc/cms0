...
- **Backup** — capture content *and all of its metadata* so it can be
  restored exactly, including identity (UUID) and access control.
...
today is preserved unchanged; backup and restore are added *around* it
...
- **Lossless by default.** The format records everything the repository
  can express. Narrowing happens at *restore* time through explicit
  options, never by silently dropping data at *export* time.
- **The ZIP is still just files.** Backup metadata lives in a reserved
  sidecar directory, so a CMS Archive opened in any unzip tool still
  presents the natural content tree. Casual download keeps working.
...
- **Identity is first class.** Backup/restore is only meaningful if a
...
**`.cms-archive/`** directory holding the metadata needed to restore it.
...
│   └── images/
│       └── cover.png
└── .cms-archive/                  ← restore metadata (ignored by casual users)
...
`roots` are the top-level selected paths, recorded so a restore can
...
constant memory — a backup of a large subtree never has to be held whole
in RAM.
...
still **records** the information (so existing backups become fully
faithful the day the capability lands) but the importer's behaviour today
is documented honestly.

| Capability | Core status | In the archive | On restore today |
| --- | --- | --- | --- |
| Properties (all types) | Implemented | Recorded | Restored exactly |
| Primary type / mixins | Implemented (`addMixin`, …) | Recorded | Restored exactly |
| References / weak refs | Implemented (depends on UUID) | Target UUID | Restored; remapped in new-UUID mode |
| ACL | Implemented (`set/getPolicy`) | `acl.ndjson` | Restored when enabled |
...
A backup that cannot bring a node back as *the same node* is not a
backup. Today the core has no way to do this: `importXML` throws,
identity comes from the database `item_id`, and there is no
`IMPORT_UUID_*` behaviour. Restoring would mint new UUIDs and break every
reference.
...
1. **Create-with-identity (core API).** A new
   ...
   restore/import code; the standard `addNode` still mints a fresh id.
...
3. **Create-with-creation-time (core API).** A further overload
   ...
   **protected** `jcr:created` at creation time (the only point it can be
   written), so a restore can bring a node back with its original creation
...
The two restore identities the product exposes map onto this:

- **Restore (preserve identity)** → create-with-identity, with `THROW` or
...
1. **Pass 1 — materialise.** Walk `nodes.ndjson` in order. Create each
   ...
   UUID per mode), restore the file body from the content tree, and set
...
2. **Pass 2 — link.** Resolve every deferred reference. In preserve mode
   ...
   the repository, otherwise surfaced as a restore warning rather than
   silently dropped).
...
| Restore ACL | on / off | off | Offered only if the archive `contains.acl`. Lets a restore deliberately drop access control. |
| Dry run | on / off | off | Validate and report (counts, conflicts, dangling references, missing types) without writing. |

"Restore ACL = off" is the explicit answer to *"restore everything except
permissions"*: ACL is always captured, and dropping it is a restore-time
...
### Import / restore (new, mirrors the export shape)

The archive is uploaded with the existing multipart-upload mutations,
then handed to a restore job that reports through the same
...
input StartImportArchiveInput {
  ...
  identity: RestoreIdentity = PRESERVE     # PRESERVE | NEW
...
| Import worker | new `…/job/archive/ImportArchiveJob.java` | Two-pass restore + ACL pass; honours all restore options; emits `jobProgress`. Symmetric to `ArchiveJob`. |
...
- **Download** stays one click; an *Include metadata* affordance (and, for
  privileged users, *Include permissions*) turns a plain download into a
  backup. `content-archive.ts` passes the new options to
...
- Import is a privileged operation: creating nodes, writing properties,
  and — for preserve/replace and ACL restore — `jcr:nodeTypeManagement`
...
- The content tree is written identically with or without the sidecar, so
  ...
  `includeMetadata` defaults to **true** — every download is restorable by
  default, directly fixing the original "properties disappear" problem;
...
- `manifest.version` gates the importer: an unknown version is refused up
  ...
  importers can detect, and decline, what they cannot restore.
- The format is independent of database schema and JCR internals; an
  archive taken from one installation restores into another of the same
...
3. **Restore.** `ImportArchiveJob` (two-pass + reference remap),
   import GraphQL mutations, dry run.
4. **ACL.** `acl.ndjson` on export, ACL pass on import, the
   include/restore toggles.
5. **Webtop restore flow.** Upload, manifest summary, options, dry-run,
   execute, progress.
